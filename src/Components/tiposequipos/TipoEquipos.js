...
export default function TipoEquipos() {
  ...
  const borrarTipoEquipo = async (e) => {
    ...
    try{
      ...
      Swal.fire({
        title: "Eliminar",
        text: "Desea eliminar el tipo de equipo?",
        icon: "warning",
        ...
        if (result.isConfirmed) {
          borrarTipoEquipoPorId(id).then((res) => {
            if (res.status === 204) {
              Swal.fire({
                title: "Eliminado",
                icon: "success",
                text: "Tipo de equipo eliminado"
              })
              listTipoEquipos();
            } else {              
              Swal.fire({
                title: "Error",
                icon: "error",
                text: "Error al aliminar el tipo de equipo"
              })
            }
          })
        }
